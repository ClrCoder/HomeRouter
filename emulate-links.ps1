...
if (!$WhatIfPreference) {
    ...
    $config = Get-Content "$PSScriptRoot/config.json" | ConvertFrom-Json -AsHashtable -Depth 10
    $config.wan.phyMacAddress = $macAddress
}
$config | ConvertTo-Json -Depth 10 | Out-File "$PSScriptRoot/config.json" -Force
